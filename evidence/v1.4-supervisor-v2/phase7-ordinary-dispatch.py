...
Filesystem: none/read/workspace (default workspace). Shell, network, browser, gui,
vision and other: host (default, available session tools) or none (explicit category
restriction across all tools). Host tools serve the outcome; Runtime cannot intercept
them or guarantee availability. Network https_get and browser render retain legacy
bounded utility-only modes; use host for general web/browser work. Optional
network_urls bounds exact HTTPS URLs, including host use when supplied; Runtime fetch
requires this list. Static render and scoped filesystem operations remain optional
utilities. Do not translate missing mechanical enforcement into a tool prohibition.
Preserve genuine task restrictions; never imply OS isolation. Set read_paths for inputs;
