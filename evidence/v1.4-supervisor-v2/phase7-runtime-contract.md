...
Runtime constructs ordinary dispatches from semantic proposals and enforces authorization.
Codex is the Supervisor. GLM is the Executor. During ordinary turns, the prompt builder
replaces the legacy Task dispatch section below with the semantic proposal contract
from scripts/ordinary_dispatch.py. The legacy section remains for Human Decision/FV.
Never use GUI/browser control, Computer Use, mouse/keyboard simulation, ZCode CLI,
or polling to route work to the Executor. Within its authorized task, the Executor
may use its available native host tools. Author the outcome and genuine restrictions;
lack of Runtime tool interception is an assurance limit, not an automatic prohibition.
See docs/v1.4-host-native-executor.md. Runtime task authority and completion remain
...
## v1.4 Phase 3 Executor finish adapter

The low-level Executor publication/staging requirements above remain the wire
compatibility contract. With the v1.4 permanent Executor prompt, entry provides
the task and finish accepts semantic artifacts, findings/evidence, outcome,
limitations and task-specific completion information. Runtime constructs hashes,
publication calls, manifests, receipt identities and staging through the existing
fence/commit APIs. Do not ask the Executor to reconstruct this packaging. Every
finish result directs STOP; only the existing completion ledger authorizes consume
and seal. Supervisor judgment, context assembly and final acceptance are unchanged.
See [the Phase 3 report](../docs/v1.4-runtime-owned-completion.md).
