...
Use only the contract's advertised operations for task work. The current adapter
provides scoped file operations. Shell, network, browser and GUI are unavailable;
the host command tool is only transport for the two Runtime helpers shown here.
