...
MODES = {"filesystem": {"none", "read", "workspace"}, "shell": {"none", "host"},
         "network": {"none", "host", "https_get"}, "browser": {"none", "host", "render"},
         "gui": {"none", "host"}, "vision": {"none", "host"}, "other": {"none", "host"}}
...
def describe(policy, *, check_host=False):
    ...
    result = {}
    for name in ("shell", "network", "browser", "gui", "vision", "other"):
        mode = modes[name]
        result[name] = {"mode": mode, "available": mode != "none", "assurance": "unavailable",
                        "operations": []}
        if mode == "none":
            result[name]["reason"] = "Explicit task restriction; do not use this category through another tool"
        if mode == "host":
            result[name].update(available="session_dependent", assurance="cooperative_unverified",
                task_work_authorized=True,
                enforcement="Host permissions apply; Runtime does not intercept native calls or attest session tools")
...
def host_inventory():
    ...
    return {"schema_version": 1, "source": "read-only installation discovery",
            ...
            "zcode": {"bundle_present": (base / "zcode.cjs").is_file(),
                ...
                "assurance": "cooperative_unverified", "task_work_authorized": "requires_READY_task",
